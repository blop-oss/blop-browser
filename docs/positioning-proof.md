...
5. Export the bounded action trace and verify ordered read, successful write,
   failed stale-reference, target-reference, session, and agent evidence.
6. Close the persistent session, export its trace while offline, and verify the
   close lifecycle event plus bounded JSON and human-timeline files.
7. Reopen that session and confirm that browser storage and trace sequence both
   continue across the restart.
8. Inspect the reported profile mode, storage scope, paths, owner, expiry, and
   destruction contract.
9. Start a disposable read-only session, confirm that a pointer action is
   denied before dispatch, and verify that close removes its managed state.
10. Destroy both persistent sessions and confirm that their managed profiles are
    gone while the response preserves destroy lifecycle evidence.
...
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Bounded semantic observations     | [Reproducible local proof](../scripts/demo-positioning-proof.mjs#L94-L127) and [reference bounding and invalidation](../src/tools/references.ts#L261-L284)                                                                                                                        | The caller chooses a limit of 1-60 exposed controls. Omitted counts preserve evidence that the view was compressed. A ref remains valid only for the exact exposed element and current page state; this is not a full DOM or unrestricted evaluation interface.                                                                                          |
| Warm cross-process sessions       | [Cross-process daemon proof](../scripts/demo-positioning-proof.mjs#L129-L169) and [daemon reuse contract](../src/cli.ts#L589-L671)                                                                                                                                                | “Warm” means later CLI processes reuse the same live named daemon, page, and in-memory refs. It is a lifecycle statement, not a browser-startup or command-latency claim.                                                                                                                                                                                |
| Parallel managed isolation        | [Concurrent path proof](../scripts/demo-positioning-proof.mjs#L49-L92), [browser-storage proof](../scripts/demo-positioning-proof.mjs#L364-L377), and [per-session directory derivation](../src/session/scope.ts#L33-L67)                                                         | Different names receive different managed profile, download, and artifact paths by default. This does not claim operating-system, network, or container isolation.                                                                                                                                                                                       |
| Profile lifecycle and scope       | [Persistence proof](../scripts/demo-positioning-proof.mjs#L269-L362), [disposal and destruction proof](../scripts/demo-positioning-proof.mjs#L379-L497), [inspectable scope type](../src/session/scope.ts#L8-L67), and [destroy implementation](../src/cli.ts#L712-L765)          | Persistent mode retains managed browser storage and its action trace after close; disposable mode removes managed state on close. `destroy` removes only known session paths. This does not preserve arbitrary server-side application state.                                                                                                            |
| Explicit existing-Chrome reuse    | [Attachment authorization gate](../src/cli.ts#L589-L639), [CDP connection implementation](../src/cli/runtime.ts#L98-L124), and [local CDP regression test](../test/cli/cli.test.ts#L880-L972)                                                                                     | Existing-profile access is opt-in through `--attach-existing`. The loopback proof deliberately does not attach to a user profile or claim isolation for that external profile.                                                                                                                                                                           |
| Chromium and Camoufox portability | [Managed launch implementations](../src/cli/runtime.ts#L98-L153) and [warm container APIs](../src/index.ts#L68-L83)                                                                                                                                                               | Chromium is the default. Camoufox is an optional third-party browser for authorized compatibility testing; support does not promise identical rendering, anonymity, or bypass of site controls.                                                                                                                                                          |
| Inspectable action evidence       | [Ordered CLI export proof](../scripts/demo-positioning-proof.mjs#L210-L362), [bounded trace recorder](../src/trace-recorder.ts#L124-L290), [CLI persistence and lifecycle integration](../src/cli/runtime.ts#L231-L374), and the [action trace contract](action-traces.md#L1-L21) | The CLI exports a bounded ordered harness action trace as JSON or a human timeline, including success, failure, lifecycle, identity, read/write, target-ref, and content-boundary evidence. Persistent sessions retain it across close. This is harness action evidence, not native Playwright tracing, video, DOM snapshots, network bodies, or replay. |
| Safety and provenance boundary    | [Read-only and disposal proof](../scripts/demo-positioning-proof.mjs#L379-L472) and [static policy and content-boundary enforcement](../src/tools/safety.ts#L16-L331)                                                                                                             | The harness denies statically classified interactions before Playwright dispatch and labels browser-derived content untrusted. It does not understand business consequences, prevent prompt injection, or sandbox page loading.                                                                                                                          |
| Framework-neutral embedding       | [Public TypeScript exports](../src/index.ts#L3-L95) and [tool factory contract](../src/create-tools.ts#L48-L275)                                                                                                                                                                  | The package supplies browser tools, session primitives, and evidence hooks. The host still owns the model loop, prompts, approval UI, reporting, uploads, and task policy.                                                                                                                                                                               |
