...
| [Published npm package](https://registry.npmjs.org/@blopai%2Fbrowser-harness/latest) | `@blopai/browser-harness` 0.1.10 is a public MIT package whose homepage and repository point back to this repository.                                                                                                                                                                                                                                                                                                                                                 | npm `latest` registry metadata for version 0.1.10.                                               | August 14, 2026 |
...
| Camoufox               | Optional Camoufox runs as a separately installed local browser or in a self-operated warm container. Anti-bot mode is off by default and launches that backend when enabled. See the [launch paths](../src/cli/runtime.ts#L82-L210) and [container test](../test/session/camoufox-container.test.ts#L21-L51).                                                                                                                                                                                                                                                                                                           | Not offered by this project (N/A). | Not offered by this project (N/A). |
...
| Privacy and telemetry  | The CLI emits a structured privacy summary before the first browser command and through `status` and `doctor`. First-party harness telemetry is off and has no collection backend; this is not a claim about browser/site, CDP, diagnostic, host/provider, or other network flows, which remain at their stated owner boundary. See the [privacy summary implementation](../src/cli/privacy.ts#L1-L70), [startup, status, and doctor tests](../test/cli/cli.test.ts#L82-L205), and [source-to-sink contract tests](../test/cli/privacy-policy.test.ts#L18-L103).                                                        | Not offered by this project (N/A). | Not offered by this project (N/A). |
...
| Retention and deletion | Persistent managed state remains until `destroy`. Disposable close or idle shutdown removes the managed profile, downloads, artifacts, and daemon log after shutdown is confirmed; a cleanup timeout preserves managed data and reports failure. `data list` reports bounded no-follow metadata, not profile contents or a complete disk inventory. Attached Chrome storage remains external, and filesystem removal is not verified secure erasure. See [cleanup lifecycle](../src/cli.ts#L884-L990), [bounded inventory](../src/cli/data-store.ts#L1-L205), and [lifecycle tests](../test/cli/cli.test.ts#L304-L485). | Not offered by this project (N/A). | Not offered by this project (N/A). |
...
profile, downloads, artifact directory (including traces, metrics, and
screenshots), and daemon log must be removed automatically after confirmed
close. A cleanup timeout reports failure and preserves the managed paths for
diagnosis. Use `--attach-existing` only after you choose to grant access to a
specific CDP browser profile.
