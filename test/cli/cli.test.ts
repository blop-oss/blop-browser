...
describe("blop-browser CLI", () => {
  ...
  test("preserves a disposable daemon log when shutdown cannot be confirmed", async () => {
    ...
    try {
      const closed = await runCliResult([
        ...
        BLOP_BROWSER_CLEANUP_TIMEOUT_MS: "25",
      });
  ...
  test("preserves all managed data when delete cannot confirm daemon shutdown", async () => {
    runtimeDir = await mkdtemp(join(tmpdir(), "blop-browser-delete-timeout-"));
    session = `delete-timeout-${process.pid}`;
    const retainedPaths = [
      join(runtimeDir, `${session}-profile`),
      join(runtimeDir, `${session}-downloads`),
      join(runtimeDir, `${session}-artifacts`),
    ];
    await Promise.all([
      ...retainedPaths.map((path) => mkdir(path)),
      writeFile(join(runtimeDir, `${session}.log`), "diagnostic evidence"),
    ]);
    const fakeDaemon = await startFakeDaemon(runtimeDir, session, "persistent");
    try {
      const deleted = await runCliResult([
        "data",
        "delete",
        session,
        "--json",
      ...
      expect(deleted.response.result).toBeUndefined();
      expect(deleted.response.error).toEqual(expect.objectContaining({
        code: "cleanup_timeout",
        message: expect.stringContaining("managed data and daemon log were preserved"),
      }));
      for (const path of retainedPaths) expect(await pathExists(path)).toBe(true);
      expect(await readFile(join(runtimeDir, `${session}.log`), "utf8"))
